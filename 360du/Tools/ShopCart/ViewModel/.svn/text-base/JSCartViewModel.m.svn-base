...
-(void) getReloadData{
    ...
    [twoPacking getUrl:[NSString stringWithFormat:SHOPPINGCARLIST,[StoreageMessage getMessage][2],@"2",[StoreageMessage getCityId]] andFinishBlock:^(id getResult) {
        NSLog(@"%@",getResult);
        NSArray *arrAll = getResult[@"datas"];
        NSInteger allCount = arrAll.count;
//        self.cartData =  [NSMutableArray arrayWithCapacity:allCount];
...
//            NSLog(@"%@",dic[@"shopName"]);
            NSInteger goodsCount = [getResult[@"prosArray"] count];
            NSMutableArray *goodsArray = [NSMutableArray arrayWithCapacity:goodsCount];
            JSCartModel *model = [[JSCartModel alloc] init];
//            NSLog(@"%@",dic[@"shopName"]);
            [model setValue:dic[@"shopName"] forKey:@"p_name"];
            [self.sectionData addObject:model];
            for (NSDictionary *dic1 in dic[@"prosArray"]) {
            
                JSCartModel *cartModel = [[JSCartModel alloc] init];
                [cartModel setValuesForKeysWithDictionary:dic1];
                [goodsArray addObject:cartModel];
                [cartModel setValue:@"999" forKey:@"p_stock"];
//                NSLog(@"%ld",(long)cartModel.p_stock);
                }
                [storeArray addObject:goodsArray];
                [shopSelectArray addObject:@""];
//            }
        }
        
        self.cartData = storeArray;
        self.shopSelectArray = shopSelectArray;
//        NSLog(@"9999999::%@,%@",self.cartData,self.sectionData);
...
- (void)getData{
    ...
    for (int i = 0; i<allCount; i++) {
        ...
        [storeArray addObject:goodsArray];
        [shopSelectAarry addObject:@"121212"];
    }
    self.cartData = storeArray;
...
- (void)selectAll:(BOOL)isSelect{
    ...
    self.cartData = [[[[self.cartData rac_sequence] map:^id(NSMutableArray *value) {
        return  [[[[value rac_sequence] map:^id(JSCartModel *model) {
            ...
            if (model.isSelect) {
                allPrices += model.unitPrice*model.count;
            }
            return model;
        }] array] mutableCopy];
    }] array] mutableCopy];
