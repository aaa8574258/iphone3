...
//    NSLog(@"%@",list);
    ...
    [cell updateDay:list outDate:self.OutDateArray selected:[self.selectedData componentsJoinedByString:@""].integerValue currentDate:[self timeString:newDate many:0] andYRDate:self.timeCannotChooseArr];
    ...
    for (NSArray *arr in self.arr) {
        self.timeCannotChooseArr = arr;
        if ([list isEqualToArray:self.timeCannotChooseArr]) {
            cell.number.backgroundColor = [UIColor whiteColor];
            cell.number.textColor =YJCorl(200, 200, 200);
            cell.stateLabel.text = @"已预订";
...
//        [self showAlertController];
    ...
    else
    {
        ...
        if (p_0 < p_1 )
        {
            ...
            [SVProgressHUD showErrorWithStatus:@"选择退房时间不能小于入住时间!" duration:1.5];
            return;
    ...
    NSInteger p_2 =[self.selectedData componentsJoinedByString:@""].intValue;
    NSInteger p_0 =[self.OutDateArray componentsJoinedByString:@""].intValue;
    NSInteger p_y =[self.timeCannotChooseArr componentsJoinedByString:@""].intValue;
    
    if (p_2 == p_0) {
        [SVProgressHUD showErrorWithStatus:@"选择退房时间不能小于入住时间!" duration:1.5];
        self.selectedData = nil;
        self.OutDateArray = nil;
    }
    
    
    if (p_y>p_0 && p_y < p_2) {
        [self.view makeToast:@"选择时间段内已有预约，请重新选择时间"];
        self.OutDateArray = nil;
        self.selectedData = nil;
    }
