...
- (instancetype)initWithFrame:(CGRect)frame andViewArr:(NSArray<UIView *> *)viewArr andTitleArr:(NSArray <NSString *>*)titleArr lineHeight:(CGFloat)lineHeight andView:(UIView *)view
{
    if (self = [super init]) {
        self.frame = frame;
        self.view = view;
...
- (UIScrollView *)itemScroll
{
    if (_itemScroll == nil) {
        ...
        for (int i = 0; i < self.titleArr.count; i++) {
            ...
            NSLog(@"%f",SCREEN_HEIGHT - self.frame.origin.y - _itemScroll.frame.size.height);
