

/********************* 有任何问题欢迎反馈给我 liuweiself@126.com ****************************************/
/***************  https://github.com/waynezxcv/Gallop 持续更新 ***************************/
/******************** 正在不断完善中，谢谢~  Enjoy ******************************************************/


#import "StatusModel.h"

@implementation StatusModel

- (id)initWithDict:(NSDictionary *)dict {
    self = [super init];
    if (self) {
        self.type = dict[@"type"];
        self.avatar = [NSURL URLWithString:dict[@"headImg"]];
        self.content = dict[@"content"];
        self.detail = dict[@"title"];
        self.date = [NSDate dateWithTimeIntervalSince1970:[dict[@"date"] floatValue]];
        self.imgs = dict[@"imgs"];
        self.name = dict[@"nickName"];
        self.statusID = dict[@"memberId"];
        
        NSMutableArray *arr2 = [NSMutableArray arrayWithCapacity:0];
        for (NSDictionary *dic in dict[@"commentList"]) {
            NSDictionary *dic3 = @{@"from":dic[@"from_nickName"],
                                   @"to":dic[@"to_nickName"],
                                   @"content":dic[@"commentContent"]};
            [arr2 addObject:dic3];
        }
        self.commentList = arr2.mutableCopy;
        
//        NSMutableArray *arr3 = [NSMutableArray arrayWithCapacity:0];
//        for (NSDictionary *dic in dict[@"commentList"]) {
//            NSDictionary *dic3 = @{@"from":dic[@"from_memberId"],
//                                   @"to":dic[@"to_memberId"],
//                                   @"mcid":dic[@"mcid"]};
//            [arr3 addObject:dic3];
//        }
        self.commentListIds = dict[@"commentList"];
        
        
        
        
        NSMutableArray *arr = [NSMutableArray arrayWithCapacity:0];
        for (NSDictionary *dic1 in dict[@"Likelist"]) {
            [arr addObject:dic1[@"nickName"]];
        }
        self.likeList = arr.mutableCopy;
        self.isLike = [dict[@"isLike"] boolValue];
        NSLog(@"%@",dict[@"ccid"]);
        self.ccid = dict[@"ccid"];
    }
    return self;
}


- (id)copyWithZone:(NSZone *)zone {
    StatusModel* one = [[StatusModel alloc] init];
    one.type = [self.type copy];
    one.avatar = [self.avatar copy];
    one.content = [self.content copy];
    one.detail = [self.detail copy];
    one.date = [self.date copy];
    one.imgs = [self.imgs copy];
    one.name = [self.name copy];
    one.statusID = [self.statusID copy];
    one.commentList = [self.commentList copy];
    one.likeList = [self.likeList copy];
    one.isLike = self.isLike ;
    one.ccid = self.ccid;
    one.commentListIds = [self.commentListIds copy];
    return one;
}


@end
