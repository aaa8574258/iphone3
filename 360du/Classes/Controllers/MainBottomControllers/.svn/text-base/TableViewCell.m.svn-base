...
- (void)extraAsyncDisplayIncontext:(CGContextRef)context
    ...
    if (!isCancelled()) {
        ...
        if ([[NSString stringWithFormat:@"%@",self.cellLayout.statusModel.type] isEqualToString:MESSAGE_TYPE_WEBSITE]) {
            self.webBtn.frame = self.cellLayout.websitePosition;
...
- (void)_layouSubViews {
    ...
    self.deleateButton.frame = self.cellLayout.deletBtnRect;
...
- (UIButton *)deleateButton {
    if (!_deleateButton) {
        NSLog(@"%@",NSStringFromCGRect(self.cellLayout.deletBtnRect));
        
        _deleateButton = [UIButton buttonWithType:UIButtonTypeCustom];
        [_deleateButton setTitle:@"删除" forState:UIControlStateNormal];
//        _deleateButton.backgroundColor = [UIColor redColor];
        [_deleateButton setTitleColor:[UIColor colorWithRed:150.0f/266 green:169.0f/266 blue:204.0f/266 alpha:1.0] forState:UIControlStateNormal];
//        [_deleateButton addTarget:self action:@selector(deleateAction) forControlEvents:UIControlEventTouchUpInside];
    }
...
//        [_webBtn setTitle:@"删除" forState:UIControlStateNormal];
        //        _deleateButton.backgroundColor = [UIColor redColor];
        [_webBtn setTitleColor:[UIColor colorWithRed:150.0f/266 green:169.0f/266 blue:204.0f/266 alpha:1.0] forState:UIControlStateNormal];
        //        [_deleateButton addTarget:self action:@selector(deleateAction) forControlEvents:UIControlEventTouchUpInside];
    }
    return _webBtn;
}
