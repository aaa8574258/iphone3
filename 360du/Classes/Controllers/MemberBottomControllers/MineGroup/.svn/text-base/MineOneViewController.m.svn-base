...
@interface MineOneViewController ()<UITableViewDelegate,UITableViewDataSource>{
...
//    NSString *url = nil;
//    url = [NSString stringWithFormat:MINEGROUPBUY,[StoreageMessage getMessage][2],@"1"];
...
-(void) forIndexData{
    ...
    [twoPacking getUrl:[NSString stringWithFormat:GETORDERLISTPAGERBYSTATUS,[StoreageMessage getMessage][2],[NSString stringWithFormat:@"%ld",self.index],@"1"] andFinishBlock:^(id getResult) {
        [self hudWasHidden:self.hudProgress];
        [self.tableView headerEndRefreshing];
        [self.tableView footerEndRefreshing];
        NSLog(@"%@",getResult);
...
-(void)forData{
    AFNetworkTwoPackaging *twoPacking = [[AFNetworkTwoPackaging alloc] init];
    [twoPacking getUrl:[NSString stringWithFormat:GETORDERLISTPAGERBYSTATUS,[StoreageMessage getMessage][2],@"1",@"1"] andFinishBlock:^(id getResult) {
        [self hudWasHidden:self.hudProgress];
        [self.tableView headerEndRefreshing];
        [self.tableView footerEndRefreshing];
        NSLog(@"%@",getResult);
...
{
    if (buttonIndex == 1) {
        [WeChantViewViewController payName:_payName andMoney:[NSString stringWithFormat:@"%0.2f",_payNumber.floatValue *100] andOder:[NSString stringWithFormat:@"SCAR%@",_ccpo]];
    }else if (buttonIndex == 2){
        [WeChantViewViewController payTHeMoneyUseAliPayWithOrderId:[NSString stringWithFormat:@"SCAR%@",_ccpo] totalMoney:[NSString stringWithFormat:@"%0.2f",_payNumber.floatValue] payTitle:_payName];
        return;
    }
}
