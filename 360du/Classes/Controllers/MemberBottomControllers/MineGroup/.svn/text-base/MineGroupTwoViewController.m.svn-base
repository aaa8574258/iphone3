...
@interface MineGroupTwoViewController ()<UITableViewDelegate,UITableViewDataSource>{
...
@implementation MineGroupTwoViewController
...
//    NSString *url = nil;
//    url = [NSString stringWithFormat:MINEGROUPBUY,[StoreageMessage getMessage][2],@"3"];
////    NSLog(@"%@",url);
//    [twoPack getUrl:url andFinishBlock:^(id getResult) {
////        NSLog(@"asjdjdkad%@",getResult);
...
-(void) forIndexData{
    ...
    [twoPacking getUrl:[NSString stringWithFormat:GETORDERLISTPAGERBYSTATUS,[StoreageMessage getMessage][2],[NSString stringWithFormat:@"%ld",self.index],@"2"] andFinishBlock:^(id getResult) {
        [self hudWasHidden:self.hudProgress];
        [self.tableView headerEndRefreshing];
        [self.tableView footerEndRefreshing];
        NSLog(@"%@",getResult);
...
-(void)forData{
    AFNetworkTwoPackaging *twoPacking = [[AFNetworkTwoPackaging alloc] init];
    [twoPacking getUrl:[NSString stringWithFormat:GETORDERLISTPAGERBYSTATUS,[StoreageMessage getMessage][2],@"1",@"2"] andFinishBlock:^(id getResult) {
        [self hudWasHidden:self.hudProgress];
        [self.tableView headerEndRefreshing];
        [self.tableView footerEndRefreshing];
