...
//        NSLog(@"%@",[appendArray objectAtIndex:i]);
        ...
        NSLog(@"%@",img);
        if ([[NSString stringWithFormat:@"%@",img] rangeOfString:@"http://www"].location != NSNotFound) {
            [imageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",img]]];
        }else{
            imageView.image = img;
        }
