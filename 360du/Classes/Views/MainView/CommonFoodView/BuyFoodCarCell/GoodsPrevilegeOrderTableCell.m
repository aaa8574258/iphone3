//
//  GoodsPrevilegeOrderTableCell.m
//  360du
//
...
@interface GoodsPrevilegeOrderTableCell(){
...
@end
@implementation GoodsPrevilegeOrderTableCell
...
- (void)makeUI{
    ...
    for (NSInteger i = 0; i < 3; i++) {
        ...
        if (i == 0 || i == 2) {
            ...
            tempLab.frame = CGRectMake(WIDTH_CONTENTVIEW - 5 * self.numSingleVersion - size.width - 10 * self.numSingleVersion - 20, 5 * self.numSingleVersion + 40 * i * self.numSingleVersion + height, size.width + 10 * self.numSingleVersion, 40 * self.numSingleVersion);
            if (i == 0) {
            ...
            tempLab.frame = CGRectMake(WIDTH_CONTENTVIEW - 5 * self.numSingleVersion - size.width / 2 - 5 * self.numSingleVersion - 10 * self.numSingleVersion - 15 * self.numSingleVersion, 5 * self.numSingleVersion + 40 * i * self.numSingleVersion + height, size.width, 40 * self.numSingleVersion);
    ...
    //过期、使用
    UIImageView *imgView = [[UIImageView alloc] initWithFrame:CGRectMake( WIDTH_CONTENTVIEW - 40 * self.numSingleVersion, 10 * self.numSingleVersion, 40 * self.numSingleVersion, 30 * self.numSingleVersion)];
    imgView.image = [UIImage imageNamed:@""];
    // imgView.layer.cornerRadius = 30 * self.numSingleVersion;
    [bgImage addSubview:imgView];
    _expiredImg = imgView;
    //_expiredImg.transform  = c
    //imgView.transform = CGAffineTransformMakeRotation(M_PI / 4);
}
- (void)setModel:(MinePrivilageModel *)model{
//    coupons_layout_isshow.png 快过期
//    coupons_used.png已使用
    ...
    NSInteger isExp = [model.isExpired integerValue];
   
    if (isExp == 0) {
        
    }else if (isExp == 1){//快过期
        _expiredImg.image = [UIImage imageNamed:@"coupons_layout_isshow.png"];

    }else{//已过期
        _expiredImg.image = [UIImage imageNamed:@"已过期.png"];
        [_bgImg setTintColor:SMSColor(211, 211, 211)];
