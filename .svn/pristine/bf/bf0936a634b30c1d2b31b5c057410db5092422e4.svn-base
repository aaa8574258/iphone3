...
- (void)makeUI{
    ...
    payBtn.frame = CGRectMake(25 * self.numSingleVesion, 80 * self.numSingleVesion * 3 - 30 * self.numSingleVesion + 64, WIDTH_CONTROLLER - 50 * self.numSingleVesion, 50 * self.numSingleVesion);
...
#warning 需要问里边的参数wyBranchCode在哪里获取
            NSString *url = [NSString stringWithFormat:PROERTYGOTOPAYBUTTON,self.model.builde,self.model.UnitNo,self.model.houseNo,self.dateFormate1,self.dateFormate2,[StoreageMessage getMessage][2],self.model.hmid,self.model.xhid];
