...
//#define ADDADDRESS MAININTERFACE@"serviceServlet?serviceName=MyAccountXmlGenerator&medthodName=addressAddJson&memberID=%@&name=%@&sex=%@&xqName=%@&xqLocation=%@&address=%@&phone=%@"
   ...
   NSString *urlStr = [NSString stringWithFormat:ADDADDRESS,[StoreageMessage getMessage][2],[[tempArr[0] urlEncodeString] urlEncodeString],[[tempArr[1] urlEncodeString] urlEncodeString],[[commituty urlEncodeString] urlEncodeString],@"",[[tempArr[3] urlEncodeString] urlEncodeString],tempArr[4]];
#warning message
    NSLog(@"urlStr:%@",urlStr);
...
-(void)makeUI:(NSArray *)array{
    NSArray *temp = @[@"联系人",@"",@"收餐地址",@"门牌号",@"手机"];
    NSArray *textFieldArr = @[@"您输入您的姓名",@"",@"请选择小区、大厦或学校",@"请输入门牌号等详细信息",@"请输入您的手机号"];
    if (self.addRessArr.count != 0) {
       textFieldArr = @[self.model.name,@"",self.model.address,self.model.address,self.model.phone];
    }
    for (NSInteger i = 0; i < 5; i++) {
        ...
        if(i != 1){
            ...
            if (i == 2) {
                textField.text = self.addressStr;
            }else if (i == 4){
...
- (void)deleteAddress:(NSArray *)array{
    ...
    [delteBtn setTitle:@"删除联系人" forState:UIControlStateNormal];
