...
//    [self.contentView addSubview:self.backStarImg];
    ...
    for (NSInteger i = 0; i < 5; i++) {
        UIImageView *starImg = [[UIImageView alloc] initWithFrame:CGRectMake((60 + 50) * self.numSingleVersion + 3 * self.numSingleVersion + 15 * self.numSingleVersion * i,  22 * self.numSingleVersion + 10 * self.numSingleVersion, 15 * self.numSingleVersion, 15 * self.numSingleVersion)];
...
//    self.foreStarImg.contentMode = UIViewContentModeLeft;
    //销售数量
    self.countLable = [[UILabel alloc] initWithFrame:CGRectMake((60 + 50) * self.numSingleVersion + 3 * self.numSingleVersion + 15 * self.numSingleVersion * 5 + 10 * self.numSingleVersion, 22 * self.numSingleVersion + 10 * self.numSingleVersion, 200 * self.numSingleVersion, 15 * self.numSingleVersion)];
...
//    starImg.image = [UIImage imageNamed:@"xing02"];
   // [self.contentView addSubview:starImg];
    for (NSInteger i = 0; i < starNum; i++) {
        UIImageView *starFlImg = [[UIImageView alloc] initWithFrame:CGRectMake((60 + 50) * self.numSingleVersion + 3 * self.numSingleVersion + 15 * self.numSingleVersion * i,  22 * self.numSingleVersion + 10 * self.numSingleVersion, 15 * self.numSingleVersion, 15 * self.numSingleVersion)];
        starFlImg.image = [UIImage imageNamed:@"xing02"];
        [self.contentView addSubview:starFlImg];
    }
    if (starNum > (NSInteger)starNum) {
        UIImageView *starFlImg = [[UIImageView alloc] initWithFrame:CGRectMake((60 + 50) * self.numSingleVersion + 3 * self.numSingleVersion + 15 * self.numSingleVersion * (NSInteger)starNum,  22 * self.numSingleVersion + 10 * self.numSingleVersion, 15 * self.numSingleVersion, 15 * self.numSingleVersion)];
...
//    self.foreStarImg.frame = CGRectMake(60 * self.numSingleVersion,  22 * self.numSingleVersion, 100 * self.numSingleVersion * starNum / 5.0, 20 * self.numSingleVersion);
    ...
    //起送价
    self.priceUsualLable.text = [NSString stringWithFormat:@"起送价:%@ 配送价:%@",model.startSendPrice,model.sendPrice];
    //判断是不是图片付、票
    if(model.isFa){
