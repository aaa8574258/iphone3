...
- (void)makeFirstView{
    ...
    [removeBtn addTarget:self action:@selector(removeBtnDown:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)makeSecondView{
    ...
    //买几个
    self.buyCount = [[UILabel alloc] initWithFrame:CGRectMake(85 * self.numSingleVesion, 5 * self.numSingleVesion, 10 * self.numSingleVesion, 10 * self.numSingleVesion)];
    self.buyCount.font = [UIFont systemFontOfSize:13];
    self.buyCount.text = [NSString stringWithFormat:@"%@",self.model.limitNumber];
...
- (void)makeThirdView{
   ...
   // firstView1.backgroundColor = SMSColor(246, 246, 246);
    ...
    [confirmBtn addTarget:self action:@selector(removeBtnDown:) forControlEvents:UIControlEventTouchUpInside];
    [firstView addSubview:confirmBtn];
    confirmBtn.tag = 1300 ;
}

- (void)removeBtnDown:(UIButton *)btn{
    //tag==1100,是取消，tag = 1200,1201,减，加，tag=1300,确定
    if (![self.target isKindOfClass:[GroupRobGoodsViewController class]]) {
        return;
    }
    if (btn.tag == 1100) {
        ...
        if (btn.tag == 1200) {
            ...
            if (self.buyCount.text.integerValue == 0) {
                ...
                self.model.limitNumber = [NSString stringWithFormat:@"%ld",(self.model.limitNumber.integerValue - 1)];
...
//                [self requestUrl:url andType:0];
            ...
            if (self.buyCount.text.integerValue == self.model.limitNumber.integerValue) {
                [self makeToast:[NSString stringWithFormat:@"只能限购%@个",self.model.limitNumber]];
            }else{
                self.model.limitNumber = [NSString stringWithFormat:@"%ld",(self.model.limitNumber.integerValue + 1)];
                self.buyCount.text = self.model.limitNumber;
