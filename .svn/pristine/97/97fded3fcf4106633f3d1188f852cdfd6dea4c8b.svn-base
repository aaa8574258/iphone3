//
//  RemarkSendFoodViewController.m
//  360du
//
//  Created by linghang on 15/7/17.
//  Copyright (c) 2015年 wangjian. All rights reserved.
//

#import "RemarkSendFoodViewController.h"
#import "BuyFoodCarViewController.h"
@interface RemarkSendFoodViewController ()
@property(nonatomic,weak)UITextField *textField;
@end

@implementation RemarkSendFoodViewController
-(id)init{
    self = [super init];
    if (self) {
        [self makeNav];
...
-(void)saveAddress{
    ...
    if ([self.target isKindOfClass:[BuyFoodCarViewController class]]) {
        [self.target returnSendDeatil:self.textField.text];
...
- (void) setContent:(NSString *)content{
    ...
    if (![content isEqual:@"无"]) {
        self.textField.text = content;
        
    }
}
-(void)makeUI{
    UITextField *textField = [[UITextField alloc] initWithFrame:CGRectMake(10 * self.numSingleVesion, 64 + 10 * self.numSingleVesion, WIDTH_CONTROLLER - 20 * self.numSingleVesion, 100 * self.numSingleVesion)];
    textField.placeholder = @"请输入配送备注";
