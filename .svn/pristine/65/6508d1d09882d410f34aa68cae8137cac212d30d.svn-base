...
- (void)makeUI{
    ...
    _yellowMineyLab.textColor = [UIColor yellowColor];
    ...
    _fullMoneyLab.layer.cornerRadius = 5 * self.numSingleVersion;
    ...
    //店铺
    _shopNameLab = [[UILabel alloc] initWithFrame:CGRectMake(10 * self.numSingleVersion, 90 * self.numSingleVersion - 5 * self.numSingleVersion, WIDTH_CONTENTVIEW - 120 * self.numSingleVersion, 15 * self.numSingleVersion)];
    ...
    //理解领取
    _getLab = [[UILabel alloc] initWithFrame:CGRectMake(WIDTH_CONTENTVIEW - 80 * self.numSingleVersion, 40 * self.numSingleVersion, 100 * self.numSingleVersion, 70 * self.numSingleVersion)];
    _getLab.text = @"立即领取";
    [_bgImg addSubview:_getLab];
    _getLab.font = [UIFont systemFontOfSize:18];
